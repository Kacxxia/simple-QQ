...
class ssqp extends EventEmitter {
    constructor(testport) {
        super()
        this.availableSocketList = {}
        this.server = net.createServer((socket) => {
            this.socketInitialize(socket)
            this.listHandler(socket)
        }).listen(testport, '127.0.0.1', () => console.log(`server is listening on ${this.server.address().address}:${this.server.address().port}:${this.server.address().family}`))
    }
        ...
        try {
            ...
            if (socket) {
                ...
                await new Promise(resolve => {
                        newSocket.connect(desport, desip, () => resolve())
                    }
                )
                ...
                console.log([newSocket.localAddress, newSocket.localPort])
                return newSocket
            }
        } catch (e) {
            console.error(e)
